CREATE OR REFRESH STREAMING TABLE logistic_dp_catalog.logistic_dp_schema.silver_staff_dlt2
...
FROM STREAM(logistic_dp_catalog.logistic_dp_schema.bronze_staff2);

CREATE OR REFRESH STREAMING TABLE logistic_dp_catalog.logistic_dp_schema.silver_geotag_dlt2
...
SELECT DISTINCT
  ...
  latitude,
  longitude
FROM STREAM(logistic_dp_catalog.logistic_dp_schema.bronze_geotag2);

CREATE OR REFRESH STREAMING TABLE logistic_dp_catalog.logistic_dp_schema.silver_shipments_dlt2
...
FROM STREAM(logistic_dp_catalog.logistic_dp_schema.bronze_shipments2);
